/**
 * Advanced Vision — main analysis class.
 * All methods from UC-A04 through UC-A20.
 */
...
import type { DetectedElement, WindowInfo } from "../layers/surface.js";
...
export class AdvancedVision {
  ...
  async getClipboardImage(): Promise<Buffer | null> {
    ...
    try {
      // Use pngpaste to get image from clipboard
  ...
  async classifyClipboardContent(): Promise<ClipboardClassification> {
    ...
    try {
      ...
      const codeIndicators = [/[{}]/, /=>;?/, /function\s*\(/, /import\s+/, /const\s+|let\s+|var\s+/];
      if (codeIndicators.filter(r => r.test(trimmed)).length >= 2) {
  ...
  async auditAccessibility(screenshot?: Buffer): Promise<AccessibilityReport> {
    const buf = screenshot ?? (await this.captureBuffer());
    const issues: import("./advanced-types.js").AccessibilityIssue[] = [];
