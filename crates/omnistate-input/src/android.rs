...
pub fn move_mouse(_point: Point) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB input not yet implemented".into(),
    ))
}

pub fn click(_button: MouseButton) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB tap not yet implemented".into(),
    ))
}

pub fn double_click(_button: MouseButton) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB double-tap not yet implemented".into(),
    ))
}

pub fn scroll(_dx: i32, _dy: i32) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB swipe not yet implemented".into(),
    ))
}

pub fn key_tap(_key: Key, _modifiers: Modifiers) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB keyevent not yet implemented".into(),
    ))
}

pub fn type_text(_text: &str) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB text input not yet implemented".into(),
    ))
}

pub fn move_mouse_smooth(_from: Point, _to: Point, _steps: u32) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB smooth gesture not yet implemented".into(),
    ))
}

pub fn drag(_from: Point, _to: Point) -> OmniResult<()> {
    Err(OmniError::UnsupportedPlatform(
        "Android ADB drag not yet implemented".into(),
    ))
}
