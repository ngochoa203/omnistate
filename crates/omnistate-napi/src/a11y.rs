...
pub fn get_ui_elements() -> Result<serde_json::Value> {
    let elements =
        omnistate_a11y::get_ui_elements().map_err(|e| Error::from_reason(e.to_string()))?;

    serde_json::to_value(&elements).map_err(|e| Error::from_reason(e.to_string()))
}
...
pub fn find_element(query: String) -> Result<serde_json::Value> {
    let element =
        omnistate_a11y::find_element(&query).map_err(|e| Error::from_reason(e.to_string()))?;

    serde_json::to_value(&element).map_err(|e| Error::from_reason(e.to_string()))
}
...
pub fn get_ui_tree() -> Result<serde_json::Value> {
    let tree = omnistate_a11y::get_ui_tree().map_err(|e| Error::from_reason(e.to_string()))?;
